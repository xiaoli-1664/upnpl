...
syms r1 r2 u0 u1 u2 real
syms A [6 6] real % 这会创建 m1_1, m1_2, ... m10_10 等符号变量并组成矩阵 m

%% Step 1: 定义向量 r 中所有 monomial（共10个）
r_vec = [r1^2;r2^2;r1*r2;r1;r2;1];
...
% --- 修正开始 (Step 3) ---
monos = sym([]); % 初始化为符号列向量
for d_total = 0:5 % 总次数从 0 到 7
  for i = 0:d_total
    for j = 0:(d_total-i)
      k = d_total - i - j;
      monos(end+1,1) = r1^i*r2^j;
    end
  end
end
...
% --- 修正开始 (Step 4) ---
S2 = sym([]); S1 = sym([]); S0 = sym([]);
for i = 1:length(monos)
    mn = monos(i);
    deg_r1 = feval(symengine, 'degree', mn, r1); % 获取 mn 关于 r1 的次数
    deg_r2 = feval(symengine, 'degree', mn, r2); % 获取 mn 关于 r1 的次数

    if deg_r2 >= 3
        S2(end+1) = mn; % S1 中的元素 r1 次数 >= 3
    elseif deg_r1 >= 3
        S1(end+1) = mn;
    else
        S0(end+1) = mn; % 其他情况
...
if length(all_monomials) ~= 21 && length(monos) == 21 % 确保all_monomials也是120，通常 S_i 为行，拼接后仍为行
...
for s_mono = S0 % 迭代S0中的每个单项式 (转置为列向量迭代，或直接 S0 如果S0是行向量)
    ...
    M(row_idx, :) = coeffs_to_row(poly, all_monomials,[r1,r2]);
    row_idx = row_idx + 1;
end
for s_mono = S1
...
for s_mono = S2
    poly = expand((s_mono / r2^3) * f2); % s_mono 必须能被 r1^3 整除 (Step 4的逻辑保证)
    M(row_idx, :) = coeffs_to_row(poly, all_monomials,[r1,r2]);
    row_idx = row_idx + 1;
end

filename = 'M_sym_N2.txt';
