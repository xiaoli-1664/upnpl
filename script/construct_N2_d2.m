...
syms r [2 1] real
syms u0 u1 u2 real
syms a1_ a2_ [2 2] real
syms b1_ b2_ [1 2] real
syms c1 c2 real
...
% --- 修正开始 (Step 3) ---
monos = sym([]); % 初始化为符号列向量
for d_total = 0:3 % 总次数从 0 到 7
  for i = 0:d_total
     k = d_total - i;
     monos(end+1,1) = r1^i*r2^k;
  end
end
...
if length(all_monomials) ~= 10 && length(monos) == 10 % 确保all_monomials也是120，通常 S_i 为行，拼接后仍为行
