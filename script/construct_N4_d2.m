...
for d_total = 0:5 % 总次数从 0 到 7
  for i = 0:d_total
    for j = 0:(d_total-i)
      for k = 0:(d_total-i-j)
        n = d_total - i - j - k;
        monos(end+1,1) = r1^i*r2^j*r3^k*r4^n;
      end
    end
  end
end
...
% --- 修正开始 (Step 4) ---
S4 = sym([]); S3 = sym([]); S2 = sym([]); S1 = sym([]); S0 = sym([]);
for i = 1:length(monos)
    mn = monos(i);
    deg_r4 = feval(symengine, 'degree', mn, r4); % 获取 mn 关于 r3 的次数
...
% 校验分组是否完整
if length(S0) + length(S1) + length(S2) + length(S3) + length(S4) ~= length(monos)
    warning('S0-S3 分组后的单项式总数与 monos 数量不符，请检查 Step 4 逻辑。');
end
% --- 修正结束 (Step 4) ---

%% Step 5: 构建 A 矩阵，并按 S0, S1, S2, S3 顺序拼接 monomial basis
all_monomials = [S0, S1, S2, S3, S4].'; % S_i 是行向量，拼接后转置为列向量以匹配常见的基向量形式
...
if length(all_monomials) ~= 126 && length(monos) == 126 % 确保all_monomials也是120，通常 S_i 为行，拼接后仍为行
...
for s_mono = S0 % 迭代S0中的每个单项式 (转置为列向量迭代，或直接 S0 如果S0是行向量)
    ...
    M(row_idx, :) = coeffs_to_row(poly, all_monomials,[r1,r2,r3,r4]);
    row_idx = row_idx + 1;
end
for s_mono = S1
...
for s_mono = S4
    poly = expand((s_mono / r4^2) * f4);
    M(row_idx, :) = coeffs_to_row(poly, all_monomials,[r1,r2,r3,r4]);
    row_idx = row_idx + 1;
end

filename = 'M_sym_N4.txt';
